/* ============================================================
   data.js  —  PCB domain data (global)
   Edit these to match your board. Positions are in marker-plane
   units (the marker is 1.0 wide, centred at 0,0). Tune x/y so each
   hotspot sits over its real component when viewed in AR.
   ============================================================ */

/* Components on the Raspberry Pi (from the block diagram). */
const COMPONENTS = [
...
/* Interactive diagnostic decision tree (mirrors the repair flowchart).
   A node is either a QUESTION (q + options) or a terminal FIX (fix). */
const DIAG = {
  start:   { q:'Is the RED power LED on?',
             options:[ {label:'Yes', go:'chk_act'}, {label:'No', go:'meas_5v'} ] },

  meas_5v: { q:'Measure PP1/PP2 — is 5V present?',
             options:[ {label:'Yes', go:'chk_33'}, {label:'No', go:'tvs'} ] },

  tvs:     { fix:'D5 TVS diode likely shorted → remove/replace it, or replace the micro-USB connector.',
             focus:'d5' },

  chk_33:  { q:'Is 3.3V present at GPIO pin 1?',
             options:[ {label:'Yes', go:'chk_act'}, {label:'No', go:'pmic'} ] },

  pmic:    { fix:'Power IC fault → reflow or replace the MXL7704 PMIC.',
             focus:'pmic' },

  chk_act: { q:'What is the GREEN ACT LED doing?',
             options:[
               {label:'No activity', go:'sd'},
               {label:'Steady on', go:'soc'},
               {label:'Blinking normally', go:'hot'} ] },
  ...
  hot:     { q:'Is any component hot to the touch?',
             options:[
               {label:'WiFi/BT hot', go:'fix_wifi'},
               {label:'USB/Eth hot', go:'fix_lan'},
               {label:'CPU hot',     go:'fix_cpu'},
               {label:'None',        go:'ok'} ] },
