  ...
  "100": {
    ...
    components: [
      ...
      { id:'gpio', name:'GPIO Header (40-pin)', x:0.150, y:-0.453, z:0,
        ...
        steps:[ 'Power off the board before touching the pins.',
                'Visually inspect all 40 pins for bent, broken or bridged pins.',
                'Measure pin 1 (3.3 V) and pin 2 (5 V) against a ground pin.',
                'If a rail reads 0 V, trace it back to its regulator.',
                'Reflow any cold or cracked solder joints on the header.' ] },
      { id:'eth', name:'Ethernet Port', x:-0.643, y:0.221, z:0,
        ...
        steps:[ 'Confirm the cable and router work on another device.',
                'Check the green / amber link LEDs on the port.',
                'Inspect the RJ45 pins and the magnetics for damage.',
                'Reseat or reflow the port solder joints.',
                'Once a link LED shows, test with ping.' ] },
      { id:'usb', name:'USB Ports (4x)', x:-0.711, y:-0.297, z:0,
        ...
        steps:[ 'Check the total USB power budget (~1.2 A shared).',
                'Try a known-good device in each port.',
                'Inspect the port pins for bent contacts or debris.',
                'Reflow the USB connector solder pads.',
                'Re-test the device on a powered USB hub.' ] },
      { id:'cpu', name:'BCM2837 — Quad-Core CPU', x:0.155, y:-0.118, z:0,
        ...
        steps:[ 'Power off and let the chip cool before touching it.',
                'Check the rails for shorts BEFORE re-powering.',
                'Inspect for solder bridges around the CPU.',
                'Add a heatsink and improve airflow.',
                'Monitor temp with vcgencmd measure_temp under load.' ] },
      { id:'wifi', name:'BCM43438 — WiFi / BT', x:-0.338, y:-0.131, z:0,
        ...
        steps:[ 'Confirm WiFi is enabled in the OS settings.',
                'Check the 3.3 V supply reaching the module.',
                'Inspect the antenna and its solder joint.',
                'Reflow the shielded module if cold-jointed.',
                'Scan for networks to confirm recovery.' ] },
      { id:'power', name:'Micro-USB Power Input', x:0.532, y:0.367, z:0,
        ...
        steps:[ 'Try a known-good 5 V / 2.5 A supply and cable.',
                'Measure 5.1 V across the power input pads.',
                'Inspect the micro-USB port for bent or broken pins.',
                'Check fuse F1 / polyfuse for continuity.',
                'Reflow or replace the micro-USB connector.' ] },
      { id:'d5', name:'D5 — TVS Diode (SMBJ5.0A)', x:0.385, y:0.314, z:0,
        ...
        steps:[ 'Power off and isolate the board.',
                'Measure D5 resistance — should read open/high, not ~0 Ω.',
                'A short means it absorbed an over-voltage event.',
                'Remove or replace D5 (SMBJ5.0A).',
                'Re-check the micro-USB input for damage before re-powering.' ] },
      { id:'hdmi', name:'HDMI Output', x:0.121, y:0.345, z:0,
        ...
        steps:[ 'Try another HDMI cable and a different monitor.',
                'Confirm the monitor input source is set correctly.',
                'Check config.txt (hdmi_force_hotplug=1).',
                'Inspect the HDMI connector pins for damage.',
                'Reflow the HDMI connector solder joints.' ] }
    ]
